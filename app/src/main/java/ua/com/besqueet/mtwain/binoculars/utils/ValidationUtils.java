package ua.com.besqueet.mtwain.binoculars.utils;
...
public class ValidationUtils extends SimpleTextWatcher {
   ...
   /* public boolean isValidUserDate(String fullNameText, String emailText, String passwordText) {
        ...
        if (isFullNameEntered && isEmailEntered && isPasswordEntered) {
            ...
            setError("Not all fields entered");
        } else {
    ...
    public boolean isValidUserDate(String emailText, String passwordText,String confirmText) {
        boolean isEmailEntered = !TextUtils.isEmpty(emailText);
        boolean isPasswordEntered = !TextUtils.isEmpty(passwordText);
        boolean isConfirmEntered = !TextUtils.isEmpty(confirmText);
        boolean isPasswordEqual = passwordText.equals(confirmText);
        boolean isPasswordNotSHort = passwordText.length()>=8;

        if (isEmailEntered && isPasswordEntered && isConfirmEntered && isPasswordEqual && isPasswordNotSHort) {
            return true;
        } else if (!isEmailEntered && !isPasswordEntered &&!isConfirmEntered) {
            setError("Not all fields entered");
        }else if (!isPasswordEqual) {
            setError(2, "Wrong confirmation");
        }  else if (!isPasswordNotSHort){
                setError(1,"Password is too short (minimum is 8)");
        } else {
            setErrors(new boolean[]{isEmailEntered, isPasswordEntered,isConfirmEntered});
        }
